...
export default function ReservationTable({
  ...
  const handleSort = (columnId: string) => {
    ...
    const [column, direction] = currentSort?.split("-") ?? [null, null];
    const newDirection =
      column === columnId && direction === "asc" ? "desc" : "asc";
    const newSort = `${columnId}-${newDirection}`;
  ...
  const deleteSelectedAgreement = async () => {
    ...
    toast.success(
      response.message || "Reservaciones selecccionadas eliminadas",
      {
        description: "Se ha eliminado la lista de reservaciones correctamente",
        position: "top-center",
        classNames: {
          toast: "bg-background",
          icon: "text-green-500",
